...
function get_nav(){
	...
	if (win < 768){
		...
		if((nav.outerWidth(true) - total) < 80){
		 	const lastItem = menu.eq(menu.length - 2)
			...
			if (deletedItems.length > 0 ) {
				...
				if(nav.outerWidth(true) - (total + width) > 80 ){
					...
					total += width
					const lastogmenu = menu.eq(menu.length - 1)
					
			       	lastogmenu.before(item); 
				}
			}
		}
		const dropdown = menu.eq(menu.length - 1).outerWidth(true)
		const nav_size = nav.outerWidth(true)
		console.log(Math.floor(nav_size - dropdown) - total)
		if (Math.floor(nav_size - dropdown) - total > -90) {
			get_nav()
		}
	}
}